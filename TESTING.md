...
### **Navigation**
---
- All links have been tested for all user types in both the collapsed navbar, desktop navbar and the shop navbar and no broken links were discovered.
...
- If an authenticated user attempts to do an admin only task they are informed they do not have the  necessary permissions and directed to the homepage
### **Index**
---
...
---
#### Profile Page
- If the user does not have any favourite items saved an appropriate prompt appears else the favourite items display correctly and react correctly to database changes.
...
#### Classes Page
- A number of issues were had on the classes page in getting the classes to display in the table appropriately, initially I wanted to leave gaps in the classes timetable to better represent what might be seen in a gym. However my resulting conditional logic used a for loop over classes and with multiple classes present this lent to an issue in additional empty `<td>` elements being created and classes appearing in the wrong place on the table. I was unable to resolve this issue so created enough data items to completely fill the table as a temporary solution. 
- I initially struggled to get the classes to update appropriately when clicked as I had every element in the table as a submit button in form and handled the request in a POST request which lead to additional complications.This was resolved by adding an js AJAX request to take the class_pk and return this to the classes view. 
...
- When initially adding the favourite button had issues with duplicating users when trying to add favourites. Intial logic was to loop through favourites and compare to request.user to check existence of user however the else statement caused multiple issues. Fixed using try except and later refined to object.get_or_create() to refactor the code.
#### Shopping Bag
- All items added from product details correctly display in the shopping bag
- Products with the same id but different sizes display correctly in the shopping bag
...
#### Checkout
- If authenticated user has previously saved checkout details the checkout form fields autofill with the correct values
...
- When the save-info box is checked the users details are correctly saved
...
---
### HTML
- All pages pass HTML validation using the passing through the page urls [HTML W3 validator](https://validator.w3.org/)
...
### JavaScript
- I tested my javascript using [JS hint](https://jshint.com/) and no errors were found
### PEP8
...
-	to immediately understand the sites purpose
    - Achieved - upon entry to the site it is clear it is a gym site
-	site navigation to be intuitive.
    - Achieved - font awesome icons and appropriate navigation buttons/bars allow for easy navigation around the site
-	to be able to see the available products
    - Achieved - store shows all products
-	to be able to search the site for specific products
    - Achieved - A search bar functions for all products
-	to be able to view individual product details
    - Achieved - Clicking on a product in the store provides additional details
-	to be able to easily view the shopping bag
    - Achieved - the cart icon clearly indicates the cart link and a preview of the bag is given in a toast
-	to be able to easily to adjust product quantities in the shopping bag
    - Achieved - increment and decrement buttons allow for this
-	to be able to easily remove products form the shopping bag
    - Achieved - text under the product quantity allows you to easily remove a product in your bag or update it's quantity
-	to be given feedback when a bag adjustment is made
    - Achieved - toasts display whenever a bag  adjustment is made
-	to receive order confirmation
    - Achieved - an email is sent to provide order conformation as well as the checkout-success view
-	to be able to easily create an account
    - Partially Achieved - minimal information required from the user to create an account but one click sign up is not enabled as I did not think it applicable to a gym website
-	to understand the benefits of being a registered user
    - Achieved - the membership page clearly explains the benefits of signing up to the site
-	the benefits of subscribing to the gym to be clear
    - Achieved - the membership page clearly explains the benefits of purchasing membership
...
-	To be able to log in and out easily
    - Achieved - clear navigation to login and logout links
-	To be able to reset my password if needed
    - Achieved - users can receive an email to reset their password if needed
-	To receive conformation I have registered for the site
    - Achieved
-	To have a personalised profile
    - Achieved - the user profile dispalys the classes the user has signed up too and their favourite items from the store
-	To save/remove favourite products to/from my profile
    - Partially Achieved - users can add and remove favourite products but not directly from their profile page
-	To save my delivery details for faster checkout
    - Achieved - users can save their delivery details on their profile page or checkout page
-	To be able to update my delivery details
    - Achieved - users can update their delivery details on thier profile page
...
-	My store discount to be obviously visible
    - Achieved - visible on the membership page and in both the checkout and shopping bag views
-	To be able to sign up to a range of classes at the gym
    - Achieved - able to sign up to a range of classes with the click of a button
-	To easily be able to change the class I am signed up to
    - Achieved - able to remove yourself from class with the click of a button
-	To be reminded by email on the morning of a class
    - Failed - out of scope
### Super User
As a superuser I want…
-	To be easily able to add/edit and delete products
    - Achieved - product management allows users to add products and they can be edited/deleted in the store and product details views
...
-	To be able to view and manage site users
    - Partially Achieved - can mange site users through admin
